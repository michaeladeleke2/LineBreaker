...
def fetch_all_lineups(force_refresh: bool = False) -> pd.DataFrame:
    ...
    for event in events:
        ...
        if str(status_num) in ("2", "3"):  # live or final
            ...
            # Upcoming game — use depth charts as best estimate
            for team in teams:
                team_id   = team.get("id", "")
                team_abbr = team.get("team", {}).get("abbreviation", "")
                depth     = _get(ESPN_DEPTHCHART.format(team_id=team_id))
                positions = depth.get("depthCharts", [])

                starters_found = set()
                for pos_group in positions:
                    athletes = pos_group.get("athletes", [])
                    for rank_info in athletes:
                        rank = rank_info.get("rank", 99)
                        name = rank_info.get("athlete", {}).get("displayName", "")
                        if not name:
                            continue
                        if rank == 1:
                            role = "starter"
                            starters_found.add(name)
                        elif rank == 2:
                            role = "sixth_man"
                        else:
                            role = "bench"

                        if name not in [r["player_name"] for r in rows]:
                            rows.append({
                                "player_name":       name,
                                "team_abbr":         team_abbr,
                                "is_starter":        rank == 1,
                                "role":              role,
                                "starter_confirmed": False,  # projected, not confirmed
                                "game_id":           game_id,
                            })
